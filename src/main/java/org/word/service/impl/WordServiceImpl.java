...
public class WordServiceImpl implements WordService {
   ...
   * @return
   */
  private Map<String, String> getDescFromMap(Map<String, Object> map) {
    Map<String, String> resMap = new HashMap<>(map.size());
    ArrayList<LinkedHashMap> list = (ArrayList<LinkedHashMap>) map.get("tags");
    for (LinkedHashMap item : list) {
      resMap.put((String) item.get("name"), (String) item.get("description"));
    }
    return resMap;
  }
  ...
  public List<Table> tableList() {
    ...
    try {
      // String path1 = WordServiceImpl.class.getClassLoader().getResource(swaggerUrl).getPath();
      //String s = readJsonData(path1);
      ...
      Map<String, String> descMap = getDescFromMap(map);
      //解析paths
      ...
      if (paths != null) {
        ...
        while (it.hasNext()) {
          ...
          String titleEng = String.valueOf(((List) content.get("tags")).get(0));
          String desc = descMap.get(title);
          title = desc + "(" + title + ")";
